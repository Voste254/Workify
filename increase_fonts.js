...
files.forEach(f => {
  ...
  // We need to be careful with replace order so we don't double replace
  // e.g., if we replace text-xl -> text-2xl first, then text-2xl -> text-3xl will hit it.
  // To avoid this, we replace with a temporary string first, or just use a state machine/matcher.
  ...
  // 4. Specifically adjust DM Mono and DM Sans SVG Icons to scale up slightly too if they are 12/14/16? 
  // Maybe not necessary, but if there's height/width it might look a bit small.
  // We'll leave SVGs for now, just fonts.
